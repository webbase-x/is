...
if(unit<1||unit>12||!Number.isInteger(unit))return;
...
const workbook=document.createElement('a');workbook.href=`workbook/?unit=${unit}`;workbook.textContent='✎ แบบฝึกบทนี้';workbook.style.cssText=link.style.cssText;
function attach(){const menu=document.querySelector('.unit1-menu-list');if(menu){if(unit>=4&&unit<=11&&!menu.contains(link))menu.append(link);if(!menu.contains(workbook))menu.append(workbook)}}
