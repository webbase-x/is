...
async function boot(role){
 ...
 w.structuredClone=structuredClone;w.confirm=()=>true;w.alert=()=>{};w.HTMLElement.prototype.getBoundingClientRect=()=>({left:0,top:0,width:500,height:700});w.SVGElement.prototype.getBoundingClientRect=()=>({left:0,top:0,width:500,height:700});w.SVGElement.prototype.setPointerCapture=()=>{};
...
test('menu relocates into the book menu when it loads later',async()=>{const {dom,w}=await boot('guest');const menu=w.document.createElement('div');menu.className='unit1-menu-list';w.document.body.append(menu);await new Promise(r=>w.setTimeout(r,20));assert.equal(menu.querySelectorAll('#p1ClassroomMenu').length,1);w.testApi.stop();dom.window.close()});

test('eraser strokes can be undone',async()=>{const {dom,w}=await boot('teacher');w.testApi.enableDrawing();w.testApi.setMode('pen');const svg=w.document.querySelector('.p1c-ink'),ev={clientX:50,clientY:60,pointerId:1,preventDefault(){},stopPropagation(){}};svg.onpointerdown(ev);svg.onpointermove({...ev,clientX:150});svg.onpointerup(ev);w.testApi.setMode('eraser');svg.onpointerdown({...ev,clientX:100});svg.onpointerup(ev);assert.equal(w.testApi.getStrokes().length,0);[...w.testApi.toolbar.querySelectorAll('button')].find(b=>b.textContent==='ย้อนกลับ').click();assert.equal(w.testApi.getStrokes().length,1);w.testApi.stop();dom.window.close()});
