...
const words = shuffle([
  { word:"กอง", emoji:"🪙", target:true }, { word:"ก้าง", emoji:"🐟", target:true },
  { word:"กำแพง", emoji:"🧱", target:true }, { word:"กิ่ง", emoji:"🌿", target:true },
  { word:"เข่ง", emoji:"🧺", target:true }, { word:"งวง", emoji:"🐘", target:true },
  { word:"จูง", emoji:"🤝", target:true }, { word:"ม่วง", emoji:"🟣", target:true },
  { word:"ระเบียง", emoji:"🏠", target:true }, { word:"อึ่งอ่าง", emoji:"🐸", target:true },
  { word:"ขนม", emoji:"🍪", target:false }, { word:"กบ", emoji:"🐸", target:false },
  { word:"จาน", emoji:"🍽️", target:false }, { word:"ถ้วย", emoji:"🥣", target:false },
  { word:"รถ", emoji:"🚗", target:false }, { word:"ภาพ", emoji:"🖼️", target:false },
  { word:"นิ้ว", emoji:"☝️", target:false }, { word:"ผัก", emoji:"🥬", target:false },
  { word:"ปลาวาฬ", emoji:"🐋", target:false }, { word:"สนุก", emoji:"🎉", target:false },
]);
const sentences = shuffle([
  ["ผึ้ง","ทำ","รัง"], ["พี่","ตี","ระฆัง"], ["น้อง","ถือ","ธง"], ["ตา","จูง","วัว"],
  ["ควาย","กิน","ฟาง"], ["แม่","ซื้อ","กระถาง"], ["พ่อ","ทา","สี","กำแพง"],
  ["ปลา","อยู่","ใน","บึง"], ["พี่","ปีน","ต้นไม้","สูง"], ["ยาย","ปลูก","ดอกไม้","สีม่วง"],
]);
const exitQuestions = shuffle([
  { q:"คำมาตราแม่กงมีลักษณะอย่างไร", choices:["มี ง เป็นตัวสะกด","ไม่มีตัวสะกด","มี ก เป็นตัวสะกด"], answer:"มี ง เป็นตัวสะกด" },
  { q:"คำใดอยู่ในมาตราแม่กง", choices:["ฟาง","ปลา","ลม"], answer:"ฟาง" },
  { q:"คำใดไม่ใช่คำมาตราแม่กง", choices:["ผึ้ง","แรง","นก"], answer:"นก" },
  { q:"ข้อใดเป็นคำมาตราแม่กงทั้ง 2 คำ", choices:["ธง, รัง","ปลา, ฟาง","ลม, นก"], answer:"ธง, รัง" },
  { q:"ตัวสะกดของคำว่า “กำแพง” คือข้อใด", choices:["ง","ม","ก"], answer:"ง" },
]);
const missionMeta = [
...
const state = { mission:0,index:0,score:0,answers:[],selected:[],busy:false,roundTimer:null,slide:0,classTimer:null,classSeconds:900,karaokeTimer:null,karaokeLine:0 };
...
function renderSort(){
  ...
  $("#stage").innerHTML=`<p class="instruction">ลากเฉพาะคำแม่กงลงกล่อง หรือใช้ปุ่มช่วยสำหรับจอสัมผัส</p><div id="roundClock" class="round-clock">⏱️ 10 วินาที</div>
    ...
    <div class="button-row"><button id="sendBox" class="primary-button">เป็นคำแม่กง</button><button id="passWord" class="secondary-button">ไม่ใช่คำแม่กง</button></div>`;
...
function renderResult(){
  ...
  const exitAnswers=state.answers.filter(x=>x.mission==="exit"),exitCorrect=exitAnswers.filter(x=>x.correct).length,passed=exitCorrect>=3;
  $("#stage").innerHTML=`<section class="result"><div class="result-medal">${passed?"🏆":"🌟"}</div><h2>${passed?"ผ่านเกณฑ์แล้ว เก่งมาก!":"เก่งขึ้นอีกก้าวแล้ว"}</h2><p>แบบทดสอบท้ายคาบถูก ${exitCorrect} จาก 5 ข้อ · เกณฑ์ผ่าน 3 ข้อ</p><div class="result-score">⭐ คะแนนรวม ${state.score}</div><p>${passed?"ได้รับดาวแล้ว! หนูจำได้แล้วว่า คำแม่กงมี ง เป็นตัวสะกด":"ฝึกอีกนิด แล้วลองใหม่นะ"}</p><button id="restartButton" class="primary-button" type="button">เล่นอีกครั้ง ↻</button></section>`;
...
const slideTemplates = [
  ...
  ()=>`<div class="karaoke-box"><span class="slide-icon">🎤</span><h2>เพลง <em>มาตราแม่กง</em></h2><div class="plan-video-frame"><iframe src="https://www.youtube-nocookie.com/embed/_bfkWevM0gM?rel=0&playsinline=1" title="เพลงมาตราแม่กง" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div><p>รอบแรก ร้องและปรบมือตามจังหวะ · รอบสอง ปรบมือเน้นคำที่มีเสียง ง ท้ายคำ</p></div>`,
  ...
  ()=>`<span class="slide-icon">🎮</span><h2>เริ่มภารกิจ<br><em>25 นาที</em></h2><p>ภารกิจ 1 กล่องคำแม่กง 10 นาที และภารกิจ 2 จรวดประโยคพุ่งทะยาน 15 นาที</p><div class="slide-actions"><button id="launchStudentGame" class="primary-button">เปิดเกมนักเรียน</button><a class="secondary-button" href="teacher.html">ควบคุมคาบจากจอครู</a></div>`,
  ...
  ()=>`<span class="slide-icon">🗝️</span><h2>สรุปและ<br><em>แบบทดสอบท้ายคาบ</em></h2><div class="rule-card">มาตราแม่กง คือ คำที่มี <strong>ง</strong> เป็นตัวสะกด และนำมาเรียงเป็นประโยคสื่อความหมายได้</div><p>ให้นักเรียนทำแบบทดสอบ 5 ข้อ เกณฑ์ผ่านอย่างน้อย 3 ข้อ</p><button id="launchExit" class="primary-button">เปิดเกมและทำแบบทดสอบ</button>`,
  ()=>`<span class="slide-icon">🎉</span><h2>ปรบมือให้<br><em>เพื่อนทุกคน</em></h2><div class="award-row">🥇 🥈 🥉</div><p>ฉายคะแนน มอบดาว และแจกใบงานที่ 2 สำหรับทบทวน</p><div class="slide-actions"><a class="primary-button" href="display.html">ฉายผลคะแนนท้ายคาบ</a><a class="secondary-button" href="worksheet-plan-2.html" target="_blank">เปิดและพิมพ์ใบงานที่ 2</a></div>`,
];
...
function renderSlide(){
  ...
  $("#karaokePlay")?.addEventListener("click",startKaraoke);$("#timerStart")?.addEventListener("click",startClassTimer);$("#timerReset")?.addEventListener("click",()=>{state.classSeconds=900;paintClassTimer()});
