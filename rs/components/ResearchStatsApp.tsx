...
function IocView({ imported, initial, onChange, title }: { imported?: ImportedProjectData | null; initial?: WorkspaceData; onChange: (data: WorkspaceData, result: WorkspaceData) => void; title: string }) {
  ...
  const importedItemCount = imported && needsOcrVerification ? Math.max(inferIocItemCount(imported.rows), ...detectedRatings.map((entry) => entry.item), initialRows?.length ?? 0, 5) : initialRows?.length ?? 5;
