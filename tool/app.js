...
function renderCalled() { const remaining = wheelWords.filter(word => !called.includes(word)); remainingCount.textContent = `เหลือ ${remaining.length} คำ`; calledCount.textContent = `${called.length} คำ`; calledWords.innerHTML = called.length ? called.map(word => `<span>${escapeHtml(word)}</span>`).join('') : '<p>ยังไม่มีคำที่สุ่ม</p>'; spinButton.disabled = !remaining.length || spinning; spinButton.textContent = remaining.length ? '✦ หมุนวงล้อ' : '✓ ครบทุกคำแล้ว'; }
...
spinButton.addEventListener('click', () => {
  const remaining = wheelWords.filter(word => !called.includes(word)); if (!remaining.length || spinning) return; spinning = true; renderCalled(); const selected = remaining[Math.floor(Math.random() * remaining.length)]; const slice = 360 / Math.max(1,wheelWords.length); const selectedIndex = wheelWords.indexOf(selected); rotation += 1440 + (360 - selectedIndex * slice - slice / 2); wheel.style.setProperty('--rotation', `${rotation}deg`); wheel.classList.add('spinning'); wheelHint.textContent = 'วงล้อกำลังเลือกคำ…'; [220,280,340,420].forEach((tone,index) => playTone(tone,.08,index * .16,'triangle'));
  window.setTimeout(() => { called.push(selected); selectedWord.textContent = selected; wheelHint.textContent = `คำที่ ${called.length}: ${selected}`; wheel.classList.remove('spinning'); spinning = false; playTone(660,.14,0,'sine'); playTone(880,.25,.15,'sine'); persistWheel(); renderCalled(); }, 4250);
});
